...
const ROUTES = {
  '/index.html': {
    ...
    meaning: 'הבקשה הצליחה — השרת מחזיר את התוכן המבוקש'
  },
  '/missing': {
    status: 404,
    ...
    meaning: 'המשאב לא נמצא — הנתיב שביקשת לא קיים בשרת'
  },
  '/admin': {
    status: 403,
    ...
    meaning: 'גישה אסורה — אין לך הרשאה לגשת למשאב זה'
  },
  '/old-page': {
    status: 301,
    ...
    body: '',
    meaning: 'הפניה קבועה — המשאב עבר לכתובת חדשה (ראה Location header)'
  }
}
...
export function HTTPRequestSim() {
  ...
  const handleSend = () => {
    ...
    const route = ROUTES[path] || {
      ...
      meaning: 'שגיאת שרת פנימית — משהו השתבש בצד השרת'
    }
  ...
  return (
    <div className="simulation-box http-sim" dir="rtl">
      <h4>הדמיית HTTP — בקשה ותגובה</h4>
      ...
      <div className="http-flow">
        <div className={`http-node http-client ${stage === 'building' ? 'active' : ''}`}>
          ...
          <span className="http-node-label">לקוח</span>
        </div>
      ...
      {stage !== 'idle' && (
        <div className="http-details">
          <div className="http-request-box">
            <div className="http-detail-title">📤 בקשה</div>
...
Connection: keep-alive`}
          ...
          {response && (
            <div className="http-response-box">
              <div className="http-detail-title">📥 תגובה</div>
              <pre className="http-pre" dir="ltr">
...
{response.body && `\n\n${response.body}`}
      ...
      <div className="http-hints">
        <span className="http-hint-label">נתיבים לניסוי:</span>
